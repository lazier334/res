...
## 相关资源
1. 脚本: [scan_swap.sh](res/a1-scan_swap.sh){:target="_blank" data-no-routing}
2. 网页: [opts_swap.html](res/a1-opts_swap.html){:target="_blank" data-no-routing}
...
### 二、构建kill.sh脚本
1. 长按 `opts_swap.html` 使用浏览器打开该网页文件 (可以[直接点击访](/docs/mds/图文贴/res/a1-opts_swap.html){:target="_blank" data-no-routing}问这个页面) 
...
- [核心原理](#核心原理){:data-no-routing}
- [快速操作](#快速操作){:data-no-routing}
- [完整版本](#完整版本){:data-no-routing}
  - [一、构建app列表](#一构建app列表){:data-no-routing}
  - [二、构建kill.sh脚本](#二构建killsh脚本){:data-no-routing}
  - [三、执行脚本批量强制停止app](#三执行脚本批量强制停止app){:data-no-routing}
- [优化方向](#优化方向){:data-no-routing}
  - [通用的kill.sh优化方向](#通用的killsh优化方向){:data-no-routing}
  - [不同品牌、不同系统的kill.sh优化方向](#不同品牌不同系统的killsh优化方向){:data-no-routing}
  - [用户的kill.sh优化方向](#用户的killsh优化方向){:data-no-routing}
